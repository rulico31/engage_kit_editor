          ...
          <>
            <AccordionSection title="基本情報" defaultOpen={true}>
              <div className="prop-group">
                <div className="prop-label">ID / Name</div>
            ...
            <AccordionSection title="塗り・背景" defaultOpen={true}>
              <div className="prop-group">
                ...
                <div className="prop-color-picker-wrapper">
                  <input type="color" className="prop-color-picker" value={item.style?.backgroundColor || "#ffffff"} onChange={(e) => handleStyleChange('backgroundColor', '', e.target.value)} />
                  <input type="text" className="prop-input" style={{ flexGrow: 1 }} value={item.style?.backgroundColor || ""} placeholder="transparent" onChange={(e) => handleStyleChange('backgroundColor', '', e.target.value)} />
                </div>
              </div>
              <CheckboxProp label="背景を透過しない(不透明)" checked={!item.data?.isTransparent} onChange={(v) => handleDataChange("isTransparent", !v)} />

              {/* Artboard Background Logic */}
              {item.name.startsWith("画像") && (
                <div style={{ marginTop: 12, borderTop: '1px solid #333', paddingTop: 12 }}>
                  <CheckboxProp label="アートボードの背景として設定" checked={!!item.data?.isArtboardBackground}
                    onChange={(v) => {
                  ...
                  {item.data?.isArtboardBackground && item.data.src && (
                    <button className="prop-button" style={{ marginTop: 8 }} onClick={() => onOpenBackgroundModal(item.id, item.data.src!)}>位置調整モードを開く</button>
                  )}
                </div>
              )}
            </AccordionSection>
